package cn.flizi.cloud.common.security.oauth.resource;
...
public class Oauth2ResourceSecurityConfigurer extends WebSecurityConfigurerAdapter {
    ...
    private void permitAllUrls(ExpressionUrlAuthorizationConfigurer<HttpSecurity>.ExpressionInterceptUrlRegistry registry,
        ...
        map.keySet().forEach(info -> {
            ...
            set.forEach(annotation -> {
                Optional.ofNullable(annotation).ifPresent(inner -> permitAllUrls.addAll(info.getPatternsCondition().getPatterns()));
            });
        });
    ...
    private void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException)
            throws IOException {
